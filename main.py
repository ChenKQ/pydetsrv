...
class Executor(object):
    ...
    @staticmethod
    def run():
        inputQueue = mp.Queue(maxsize=Executor.inputQuueSize)
        outputQueue = mp.Queue(maxsize=Executor.outputQueueSize)
        inferencer = Inferencer("./runs/train/exp3/weights/best.pt")

        pread = mp.Process(target=Executor.readThread, args=(inputQueue,), daemon=True)
        pwrite = mp.Process(target=Executor.writeThread, args=(outputQueue,), daemon=True)
        pread.start()
        pwrite.start()
        ...
        while True:
            ...
            outputQueue.put(dst)
            if outputQueue.qsize() > Executor.outputQueueSize-1:
                outputQueue.get()
            # if outputQueue.qsize() < 1:
            #     print("put extra")
            #     outputQueue.put(dst)
            print("output queue size: ", outputQueue.qsize())
            if not pread.is_alive:
                pwrite.terminate()
                break
            if not pwrite.is_alive():
                pread.terminate
                break
        pread.join()
        pwrite.join()
    ...
    @staticmethod
    def writeThread(imageQueue):
        server = RtspServer(1920, 1080, 30, "/live/detection", 8554, imageQueue)
        server.run()
